...
4. **Baja al mutante.** Toda fila cuya clase tenga mutantes que cambiaron de estado lleva un botón con el número de cambios; al desplegarla ves cada uno con su línea, mutador, estado antes → ahora y el tipo de cambio (nuevo superviviente, ahora detectado, sin cobertura ahora…). La casilla «Solo mutantes sin detectar» deja a la vista lo que baja la robustez: lo que antes se mataba y ahora sobrevive o se queda sin cobertura, y también los mutantes nuevos que nacen así.
...
8. **Exportar JSON** guarda la comparación en sí —no un informe, sino el resultado con sus ficheros de origen y umbrales— como `mutadiff-comparison-<id>.mutadiff.json`. En la pantalla inicial, **Importar comparación** lo abre y vuelve al mismo dashboard sin subir otra vez los reportes: sirve para recuperarla tras cerrar la pestaña o para pasársela a alguien que la quiera explorar, no solo leer. El fichero se valida al importarlo; si no es una comparación exportada, o viene de un formato que esta versión no sabe leer, la app lo dice en vez de abrir un dashboard roto.
...
- **Sin histórico.** Una comparación vive en la pestaña que la creó: sobrevive a recargar, pero no a cerrar el navegador, y no se puede compartir por enlace. Para conservar o mandar un resultado, exporta el JSON (se vuelve a abrir en la app) o el HTML (se lee sin ella). La persistencia opt-in, la atribución de autor vía `git log` y el modo CLI para integrarlo en pipelines están pendientes de decisión, no de implementación: cómo se distribuye el CLI y de dónde sale el informe base en CI son las preguntas abiertas.
...
El proyecto se somete a su propio tipo de análisis: mutation testing con Stryker sobre `packages/core` y la lógica de `packages/web/src/lib`, con el corte en 95 y el score actual en **98,84 %** (`core` al 99,70 % y `web/src/lib` al 96,00 %; los diez vivos que quedan son equivalentes documentados). La suite tiene, a día de hoy, 486 tests unitarios/integración y 13 e2e en navegador: seis sobre el flujo real, tres sobre el **artefacto ya construido** —servido como lo sirve un servidor de ficheros, desde un subpath y sin reescribir rutas, que es exactamente lo que se publica en la release— y cuatro de **accesibilidad** con axe-core sobre WCAG 2.1 AA. CI ejecuta lint, typecheck, tests y e2e en cada PR; Stryker va en un workflow aparte que se lanza al fusionar en `master` cualquier cambio en el código que mide, y la release no se publica sin pasar la misma barra. Dependabot agrupa las actualizaciones de dependencias y se lanza bajo demanda, no en un ciclo fijo.
